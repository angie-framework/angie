...
var IGNORE_KEYS = ['Controller', 'template', 'templatePath'];
var routes = {
    '/': {
...
var $RouteProvider = (function () {
    ...
    _createClass($RouteProvider, null, [{
        key: 'when',
        ...
        value: function when(path, obj) {
            ...
            Object.keys(obj).forEach(function (v) {
                ...
                if (IGNORE_KEYS.indexOf(v) > -1) {
                    ...
                    // RegExp
                    childPath = $RouteProvider.$stringsToRegExp(path, _utilUtil.$StringUtil.removeTrailingLeadingSlashes(v));
                } else {
        ...
        value: function otherwise(path) {
            if (typeof path === 'string') {
                ...
                _angieLog2['default'].warn('Cannot set "otherwise" route to anything other than a string');
            }
            return path;
        }
        ...
        value: function fetch() {
            var obj = { routes: routes };
            if (_otherwise) {
                obj.otherwise = _otherwise;
            }
            return obj;
        }
        ...
        key: '$stringsToRegExp',
        value: function $stringsToRegExp() {
            return new RegExp(Array.prototype.slice.call(arguments).map(function (v) {
                return _utilUtil.$StringUtil.removeTrailingLeadingSlashes(v.toString());
            }).join('\\/'));
        }
        ...
        key: '$$parseURLParams',
        value: function $$parseURLParams(pattern, path) {
            var obj = {};
            ...
            if (pattern && path) {
                ...
                // Strip slashes
                _utilUtil.$StringUtil.removeTrailingLeadingSlashes(path.replace(pattern, '$1|$2|$3|$4|$5')).split('|').forEach(
